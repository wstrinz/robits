...
Elm.RobitGame = Elm.RobitGame || {};
Elm.RobitGame.make = function (_elm) {
   "use strict";
   _elm.RobitGame = _elm.RobitGame || {};
   if (_elm.RobitGame.values)
   return _elm.RobitGame.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "RobitGame",
   $Basics = Elm.Basics.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var actionDisabled = F2(function (_v0,
   model) {
      return function () {
         switch (_v0.ctor)
         {case "Scavenge":
            return _U.cmp(model.scavengeTimeout,
              0) > 0;}
         _U.badCase($moduleName,
         "on line 101, column 33 to 58");
      }();
   });
   var canCombine = function (model) {
      return _U.cmp(model.parts,
      1) > 0;
   };
   var doTick = F2(function (_v2,
   model) {
      return function () {
         switch (_v2.ctor)
         {case "Tick":
            return _U.replace([["scavengeTimeout"
                               ,_U.cmp(model.scavengeTimeout,
                               0) > 0 ? model.scavengeTimeout - 1 : 0]],
              model);}
         _U.badCase($moduleName,
         "between lines 95 and 97");
      }();
   });
   var combine = F2(function (action,
   model) {
   ...
   var scavenge = F2(function (action,
   model) {
      return _U.replace([["parts"
                         ,model.parts + 3]
                        ,["scavengeTimeout",10]],
   ...
   model) {
      return function () {
         switch (action.ctor)
         {case "Combine":
            return A2(combine,action,model);
            case "Scavenge":
            return A2(scavenge,
              action,
              model);
            case "Tick": return A2(doTick,
              action,
              model);}
         _U.badCase($moduleName,
   ...
   var buttonStyle = $Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                         ...
                                                         ,{ctor: "_Tuple2"
                                                          ,_0: "padding"
                                                          ,_1: "10px"}]));
   var textDiv = function (model) {
      return A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([$Html.text("Parts: ")
                   ,$Html.text($Basics.toString(model.parts))
                   ,$Html.text(" Robits: ")
                   ,$Html.text($Basics.toString(model.robits))]));
   };
   ...
   var start = function (app) {
      return function () {
         ...
         actions.address,
         $Maybe.Just);
         var mod_op = A3($Signal.foldp,
         F2(function (_v7,modState) {
            return function () {
               switch (_v7.ctor)
               {case "Just":
   ...
   var Combine = {ctor: "Combine"};
   var combineDiv = F2(function (address,
   model) {
      return A2($Html.button,
      _L.fromArray([A2($Html$Events.onClick,
                   address,
                   Combine)
                   ,$Html$Attributes.disabled($Basics.not(canCombine(model)))
                   ,buttonStyle]),
      _L.fromArray([$Html.text("Combine")]));
   });
   var Scavenge = {ctor: "Scavenge"};
   var scavengeText = function (model) {
      return A2(actionDisabled,
      Scavenge,
      model) ? A2($Basics._op["++"],
      "Scavenge (",
      A2($Basics._op["++"],
      $Basics.toString(model.scavengeTimeout),
      ")")) : "Scavenge";
   };
   var scavengeDiv = F2(function (address,
   model) {
      return A2($Html.button,
      _L.fromArray([A2($Html$Events.onClick,
                   address,
                   Scavenge)
                   ,buttonStyle
                   ,$Html$Attributes.disabled(A2(actionDisabled,
                   Scavenge,
                   model))]),
      _L.fromArray([$Html.text(scavengeText(model))]));
   });
   var controlDiv = F2(function (address,
   model) {
      return A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([A2(scavengeDiv,
                   address,
                   model)
                   ,A2(combineDiv,
                   address,
